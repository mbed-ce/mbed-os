...
/**
  ...
  *-----------------------------------------------------------------------------
  *-----------------------------------------------------------------------------
  * Clock source        | HSE   | HSI (x6 x8 xB xC / else others)
  *-----------------------------------------------------------------------------
  * SYSCLK(MHz)         | 72    | 64/72
  * AHBCLK (MHz)        | 72    | 64/72
  * APB1CLK (MHz)       | 36    | 32/36
  * APB2CLK (MHz)       | 72    | 64/72
  * USB capable         | YES   | NO/YES
  *-----------------------------------------------------------------------------
  */
...
#if (HSE_VALUE < 6000000U) || (HSE_VALUE > 32000000U)
#error HSE_VALUE must be >= 6MHz and <= 32MHz for STM32F3 common clock config
#endif
...
uint8_t SetSysClock_PLL_HSE(uint8_t bypass)
{
    ...
    {
        error("HSE_VALUE not divisible by 6MHz or 8MHz\n");
    }
...
#if DEVICE_USBDEVICE && defined(RCC_PERIPHCLK_USB)  
...
uint8_t SetSysClock_PLL_HSI(void)
{
...
#if !defined(RCC_CFGR_PLLSRC_HSI_PREDIV)
    /* HSI devider is predefined to 2 so MCU can not reach 72 MHz */
    RCC_OscInitStruct.PLL.PLLMUL          = RCC_PLL_MUL16; // 64 MHz (8 MHz * 9)
#else
    ...
    /* Select PLL as system clock source and configure the HCLK, PCLK1 and PCLK2 clocks dividers */
    RCC_ClkInitStruct.ClockType      = (RCC_CLOCKTYPE_SYSCLK | RCC_CLOCKTYPE_HCLK | RCC_CLOCKTYPE_PCLK1 | RCC_CLOCKTYPE_PCLK2);
    RCC_ClkInitStruct.SYSCLKSource   = RCC_SYSCLKSOURCE_PLLCLK; // 72/64 MHz
    RCC_ClkInitStruct.AHBCLKDivider  = RCC_SYSCLK_DIV1;         // 72/64 MHz
    RCC_ClkInitStruct.APB1CLKDivider = RCC_HCLK_DIV2;           // 36/32 MHz
    RCC_ClkInitStruct.APB2CLKDivider = RCC_HCLK_DIV1;           // 72/64 MHz
    if (HAL_RCC_ClockConfig(&RCC_ClkInitStruct, FLASH_LATENCY_2) != HAL_OK) {
        return 0; // FAIL
    }
