...
from .pio_variants import PIO_VARIANT_TO_MBED_TARGET
from .cmake_to_scons_converter import build_library
...
# Directories
FRAMEWORK_DIR = platform.get_package_dir("framework-mbed-ce")
...
def run_cmake(src_dir: pathlib.Path, build_dir: pathlib.Path, extra_args=None):
    cmd = [
...
def get_cmake_code_model(src_dir: Path, build_dir: Path, extra_args=None) -> dict:
...
def generate_project_ld_script() -> pathlib.Path:
    ...
    run_cmake(PROJECT_DIR, BUILD_DIR, [
        "--build",
        BUILD_DIR,
        "--target",
        "mbed-linker-script"
    ...
    return next(BUILD_DIR.glob("*.link-script.ld"))
