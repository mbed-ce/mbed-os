...
def get_interface(dev: usb.core.Device, interface, alternate=0):
    intf = None
...
class PyusbBasicTest(BaseHostTest):
    ...
    def _callback_control_basic_test(self, key, value, timestamp):
        ...
        self.test_usb_device(
            ...
            test_fun=self.control_basic_test,
            log=self.log,
    ...
    def _callback_control_stall_test(self, key, value, timestamp):
        self.test_usb_device(
            ...
            test_fun=self.control_stall_test,
            log=self.log
        )
    ...
    def _callback_control_sizes_test(self, key, value, timestamp):
        self.test_usb_device(
            usb_dev_serial_number=value,
            test_fun=self.control_sizes_test,
            log=self.log
        )
    ...
    def _callback_control_stress_test(self, key, value, timestamp):
        self.test_usb_device(
            usb_dev_serial_number=value,
            test_fun=self.control_stress_test,
            log=self.log
        )
    ...
    def _callback_ep_test_data_correctness(self, key, value, timestamp):
        self.test_usb_device(
            usb_dev_serial_number=value,
            test_fun=self.ep_test_data_correctness,
            log=self.log
        )
    ...
    def _callback_ep_test_parallel_transfers(self, key, value, timestamp):
        self.test_usb_device(
            usb_dev_serial_number=value,
            test_fun=self.ep_test_parallel_transfers,
            log=self.log
        )
    ...
    def _callback_ep_test_parallel_transfers_ctrl(self, key, value, timestamp):
        self.test_usb_device(
            usb_dev_serial_number=value,
            test_fun=self.ep_test_parallel_transfers_ctrl,
            log=self.log
        )
    ...
    def _callback_ep_test_data_toggle(self, key, value, timestamp):
        self.test_usb_device(
            usb_dev_serial_number=value,
            test_fun=self.ep_test_data_toggle,
            log=self.log
        )
    ...
    def find_device(self, serial_number) -> usb.Device:
        # to make it more reliable, 20 retries in 2[s]
        for _ in range(20):
            dev = usb.core.find(custom_match=self.TestMatch(serial_number), backend=USB_BACKEND)
            if dev is not None:
                break
            time.sleep(0.1)
        return dev
    ...
    def setup(self):
        ...
        self.device_reset_test = self.device_reset_test(log=self.log)
        self.device_reset_test.send(None)

        self.device_soft_reconnection_test = self.device_soft_reconnection_test(log=self.log)
        self.device_soft_reconnection_test.send(None)

        self.device_suspend_resume_test = self.device_suspend_resume_test(log=self.log)
        self.device_suspend_resume_test.send(None)

        self.repeated_construction_destruction_test = self.repeated_construction_destruction_test(log=self.log)
    ...
    def control_basic_test(self, dev: usb.core.Device, vendor_id, product_id, log):
    
        self.log(f"Testing with device (bus = {dev.bus}, address = {dev.address})")

        self.get_set_configuration_test(dev, log)
        self.get_set_interface_test(dev, log)
        self.get_status_test(dev, log)
        self.set_clear_feature_test(dev, log)
        self.get_descriptor_test(dev, vendor_id, product_id, log)
        self.set_descriptor_test(dev, log)
    
    
    def get_set_configuration_test(self, dev: usb.core.Device, log):
        """
        ...
        self.log("<<< get_set_configuration_test >>>")
        # check if dafault(1) configuration set
        try:
            ret = usb.control.get_configuration(dev)
            self.raise_if_different(1, ret, self.lineno(), 'Invalid configuration.')
        except usb.core.USBError as error:
            self.raise_unconditionally(self.lineno(), 'get_configuration request failed ({}).'.format(str(error).strip()))
        ...
        try:
            ...
            self.raise_unconditionally(self.lineno(), 'set_configuration request (deconfigure) failed ({}).'.format(str(error).strip()))
    
        # check if deconfigured
        try:
            ret = usb.control.get_configuration(dev)
            self.raise_if_different(0, ret, self.lineno(), 'Invalid configuration.')
            self.log("device deconfigured - OK")
        except usb.core.USBError as error:
            self.raise_unconditionally(self.lineno(), 'get_configuration request failed ({}).'.format(str(error).strip()))
        ...
        for cfg in dev:
            try:
                ...
                self.raise_unconditionally(self.lineno(), 'set_configuration request failed ({}).'.format(str(error).strip()))
    
            # check if configured
            try:
                ret = usb.control.get_configuration(dev)
                self.raise_if_different(cfg.bConfigurationValue, ret, self.lineno(), 'Invalid configuration.')
                self.log("configuration {} set - OK ".format(cfg.bConfigurationValue))
            except usb.core.USBError as error:
                self.raise_unconditionally(self.lineno(), 'get_configuration request failed ({}).'.format(str(error).strip()))
            # test control data transfer after configuration set
            self.control_data_test(dev, [64, 256], log)
        self.log("") # new line
    
    
    def get_set_interface_test(self, dev: usb.core.Device, log):
        """
        ...
        self.log("<<< get_set_interface_test >>>")
        # for every configuration
        for cfg in dev:
            cfg.set()
            # for every interface
            for intf in cfg:
                ...
                self.raise_if_different(intf.bAlternateSetting, altsett, self.lineno(), text='Wrong alternate setting for interface {}'.format(intf.bInterfaceNumber))
                self.log("cfg({}) interface {}.{} set - OK".format(cfg.bConfigurationValue, intf.bInterfaceNumber, intf.bAlternateSetting))
                self.control_data_test(dev, [64, 256], log)
    
            self.release_interfaces(dev)
    
        self.restore_default_configuration(dev)
    ...
    def get_status_test(self, dev: usb.core.Device, log):
        """
        ...
        self.log("<<< get_status_test >>>")
        # check device status
        ret = self.get_status(dev, CTRL_RECIPIENT_DEVICE)
        # Status bits
        # ret == 0b01    (D0)Self Powered
        # ret == 0b10    (D1)Remote Wakeup
        # (D2 - D15 reserved) Must be set to 0
        if(ret < 0 or ret > 3):
            self.raise_unconditionally(self.lineno(), "GET_STATUS on DEVICE failed")
    
        # check endpoint 0 status
        ret = self.get_status(dev, CTRL_RECIPIENT_ENDPOINT, 0)
        # Status bits
        # ret == 0b1    (D0)endpoint Halt
        # (D1 - D15 reserved) Must be set to 0
        # endpoint 0 can't be halted ret == 0
        self.raise_if_different(0, ret, self.lineno(), "GET_STATUS on ENDPOINT 0 should return 0")
    
        # for every configuration
        for cfg in dev:
            cfg.set()
            self.raise_if_different(cfg.bConfigurationValue, usb.control.get_configuration(dev), self.lineno(), "Configuration {} set failed".format(cfg.bConfigurationValue))
    
            for intf in cfg:
                intf.set_altsetting()
                # check interface status
                ret = self.get_status(dev, CTRL_RECIPIENT_INTERFACE, intf.bInterfaceNumber)
                # Status bits
                # ret == 0b0
                # (D0 - D15 reserved) Must be set to 0
                if(ret != 0):
                    self.raise_unconditionally(self.lineno(), "GET_STATUS on INTERFACE ({},{}) failed".format(intf.bInterfaceNumber, intf.bAlternateSetting))
                self.log("cfg({}) interface {}.{} status - OK".format(cfg.bConfigurationValue, intf.bInterfaceNumber, intf.bAlternateSetting))
                ...
                for ep in intf:
                    ...
                    # (D1 - D15 reserved) Must be set to 0
                    if(ret >= 1):
                        self.raise_unconditionally(self.lineno(), "GET_STATUS on ENDPOINT {} failed - endpoint halted".format(ep.bEndpointAddress))
                    self.log("cfg({}) intf({}.{}) endpoint {} status - OK".format(cfg.bConfigurationValue, intf.bInterfaceNumber, intf.bAlternateSetting, ep.bEndpointAddress))
    ...
    def set_clear_feature_test(self, dev: usb.core.Device, log):
        """
        ...
        self.log("<<< set_clear_feature_test >>>")
        # TODO:
            ...
            for intf in cfg:
                ...
                for ep in intf:
                    ...
                    try:
                        ...
                        self.raise_unconditionally(self.lineno(), 'set_feature request (halt) failed for endpoint {} ({}).'.format(ep.bEndpointAddress, str(err).strip()))
    
                    # check if endpoint was halted
                    try:
                        ret = usb.control.get_status(dev, ep)
                    except usb.core.USBError as err:
                        self.raise_unconditionally(self.lineno(), 'get_status request failed for endpoint {} ({}).'.format(ep.bEndpointAddress, str(err).strip()))
                    if(ret != 1):
                        self.raise_unconditionally(self.lineno(), "endpoint {} was not halted".format(ep.bEndpointAddress))
                    self.log("cfg({}) intf({}.{})   ep {} halted - OK".format(cfg.bConfigurationValue, intf.bInterfaceNumber, intf.bAlternateSetting, ep.bEndpointAddress))
    
                    # Control OUT CLEAR_FEATURE on endpoint - unhalt
                    try:
                        usb.control.clear_feature(dev, FEATURE_ENDPOINT_HALT, ep)
                    except usb.core.USBError as err:
                        self.raise_unconditionally(self.lineno(), "clear_feature request (unhalt) failed for endpoint {} ({})".format(ep.bEndpointAddress, str(err).strip()))
    
                    # check if endpoint was unhalted
                    ret = usb.control.get_status(dev, ep)
                    if(ret != 0):
                        self.raise_unconditionally(self.lineno(), "endpoint {} was not unhalted".format(ep.bEndpointAddress))
                    self.log("cfg({}) intf({}.{})   ep {} unhalted - OK".format(cfg.bConfigurationValue, intf.bInterfaceNumber, intf.bAlternateSetting, ep.bEndpointAddress))
    ...
    def get_descriptor_test(self, dev: usb.core.Device, vendor_id, product_id, log):
        """
        ...
        self.log("<<< get_descriptor_test >>>")
        # device descriptor
        try:
            ret = self.get_descriptor(dev, (DESC_TYPE_DEVICE << 8) | (0 << 0), 0, DEVICE_DESC_SIZE)
            dev_desc = dict(zip(device_descriptor_keys, device_descriptor_parser.unpack(ret)))
            self.raise_if_different(DEVICE_DESC_SIZE, dev_desc['bLength'], self.lineno(), text='Wrong device descriptor size.')
            self.raise_if_different(vendor_id, dev_desc['idVendor'], self.lineno(), text='Wrong vendor ID.')
            self.raise_if_different(product_id, dev_desc['idProduct'], self.lineno(), text='Wrong product ID.')
        except usb.core.USBError:
            self.raise_unconditionally(self.lineno(), "Requesting device descriptor failed")
    
        # configuration descriptor
        try:
            ret = self.get_descriptor(dev, (DESC_TYPE_CONFIG << 8) | (0 << 0), 0, CONFIGURATION_DESC_SIZE)
            conf_desc = dict(zip(configuration_descriptor_keys, configuration_descriptor_parser.unpack(ret)))
            self.raise_if_different(CONFIGURATION_DESC_SIZE, conf_desc['bLength'], self.lineno(), text='Wrong configuration descriptor size.')
        except usb.core.USBError:
            self.raise_unconditionally(self.lineno(), "Requesting configuration descriptor failed")
    
        # interface descriptor
        try:
            ret = self.get_descriptor(dev, (DESC_TYPE_INTERFACE << 8) | (0 << 0), 0, INTERFACE_DESC_SIZE)
            self.raise_unconditionally(self.lineno(), "Requesting interface descriptor should fail since it is not directly accessible.")
        except usb.core.USBError:
            log("interface descriptor is not directly accessible - OK")
    
        # endpoint descriptor
        try:
            ret = self.get_descriptor(dev, (DESC_TYPE_ENDPOINT << 8) | (0 << 0), 0, ENDPOINT_DESC_SIZE)
            self.raise_unconditionally(self.lineno(), "Requesting endpoint descriptor should fail since it is not directly accessible.")
        except usb.core.USBError:
            log("endpoint descriptor is not directly accessible - OK")
        self.log("") # new line
    
    
    def set_descriptor_test(self, dev: usb.core.Device, log):
        """
        ...
        try:
            ...
            self.raise_unconditionally(self.lineno(), "set_descriptor request should fail since it is not implemented")
        except usb.core.USBError:
            log("SET_DESCRIPTOR is unsupported - OK")
        self.log("") # new line
    
    
    def synch_frame_test(self, dev: usb.core.Device, log):
        """
        ...
        try:
            ...
            self.raise_unconditionally(self.lineno(), "SYNCH_FRAME request failed")
        self.log("") # new line
    
    
    def control_stall_test(self, dev: usb.core.Device, log):
        """
        ...
        try:
            ...
            self.raise_unconditionally(self.lineno(), "Invalid request not stalled")
        except usb.core.USBError:
        ...
        for i in (3, 4, 5):
            try:
                ...
                self.raise_unconditionally(self.lineno(), "Requesting string failed i: " + str(i))
        ...
        for i in (6, 7):
            try:
                ...
                length = 255
                resp = dev.ctrl_transfer(request_type, request, value, index, length, 5000)
                self.raise_unconditionally(self.lineno(), "Requesting string passed i: " + str(i))
            except usb.core.USBError:
                log("Requesting string %s failed - OK" % i)
        self.log("") # new line
    
    
    def control_sizes_test(self, dev: usb.core.Device, log):
        """
        ...
        self.control_data_test(dev, list, log)
    
    
    def control_data_test(self, dev: usb.core.Device, sizes_list, log):
        ...
        for i in sizes_list:
            ...
            try:
                ...
                self.raise_unconditionally(self.lineno(), "VENDOR_TEST_CTRL_OUT_SIZES failed ")
            ...
            try:
                ...
                self.raise_if_different(i, len(ret), self.lineno(), "send/receive data is the wrong size")
                for j in range(0, i):
                    self.raise_if_different(data[j], ret[j], self.lineno(), "send/receive data mismatch")
            except usb.core.USBError:
                self.raise_unconditionally(self.lineno(), "VENDOR_TEST_CTRL_IN_SIZES failed")
            count += 1
    
    
    def control_stress_test(self, dev: usb.core.Device, log):
        """
    ...
    def find_ep_pair(self, intf, endpoint_type):
        ...
        if not all((ep_out, ep_in)):
            self.raise_unconditionally(self.lineno(), 'Unable to find {} endpoint pair.'
                                  .format(ENDPOINT_TYPE_NAMES[endpoint_type]))
        self.raise_if_different(ep_out.wMaxPacketSize, ep_in.wMaxPacketSize, self.lineno(),
                           'wMaxPacketSize not equal for OUT and IN {} endpoints.'
                           .format(ENDPOINT_TYPE_NAMES[endpoint_type]))
        return ep_out, ep_in
    
    
    def loopback_ep_test(self, ep_out, ep_in, payload_size):
        ...
        self.raise_if_different(payload_out, payload_in, self.lineno(), 'Payloads mismatch.')
    
    
    def random_size_loopback_ep_test(self, ep_out, ep_in, failure, error, seconds, log, min_payload_size=1):
        ...
        while time.time() < end_ts and not failure.is_set() and not error.is_set():
            ...
            try:
                self.loopback_ep_test(ep_out, ep_in, payload_size)
    ...
    def halt_ep_test(self, dev: usb.core.Device, ep_out, ep_in, log):
        ...
        for ep in (ep_out, ep_in):
            try:
                if (usb.control.get_status(dev, ep) == 1):
                    self.raise_unconditionally(self.lineno(), 'Endpoints must NOT be halted at the start of this test')
            except usb.core.USBError as err:
                self.raise_unconditionally(self.lineno(), 'Unable to get endpoint status ({!r}).'.format(err))
        ...
        try:
            while delayed_halt.is_alive():
                if ctrl_error.is_set():
                    break
                try:
                    self.loopback_ep_test(ep_out, ep_in, ep_out.wMaxPacketSize)
                except usb.core.USBError as err:
                    if ctrl_error.is_set():
                        break
                    try:
                    ...
                    if ep_status == 1:
                        ...
                        self.raise_unconditionally(self.lineno(), 'Unexpected error ({!r}).'.format(err))
            if ctrl_error.is_set():
                self.raise_unconditionally(self.lineno(), 'Halting endpoint {0.bEndpointAddress:#04x} failed'
                                      .format(ep_to_halt))
        finally:
            ...
            if not ctrl_error.is_set():
                ep_out.clear_halt()
                ep_in.clear_halt()
        self.raise_unconditionally(self.lineno(), 'Halting endpoint {0.bEndpointAddress:#04x}'
    ...
    @staticmethod
    def request_abort_buff_check(dev: usb.core.Device, ep):
        ctrl_kwargs = {
    ...
    def ep_test_data_correctness(self, dev: usb.core.Device, log, verbose=True):
        ...
        for intf in cfg:
            ...
            iso_out, iso_in = self.find_ep_pair(intf, usb.ENDPOINT_TYPE_ISOCHRONOUS)
            ...
            for payload_size in range(bulk_out.wMaxPacketSize + 1):
                try:
                    self.loopback_ep_test(bulk_out, bulk_in, payload_size)
                except usb.USBError as err:
                    self.raise_unconditionally(self.lineno(), USB_ERROR_FMT.format(err, bulk_out, bulk_in, payload_size))
    
            if verbose:
                log('Testing OUT/IN data correctness for interrupt endpoint pair.')
            for payload_size in range(interrupt_out.wMaxPacketSize + 1):
                try:
                    self.loopback_ep_test(interrupt_out, interrupt_in, payload_size)
                except usb.USBError as err:
                    self.raise_unconditionally(self.lineno(), USB_ERROR_FMT.format(err, interrupt_out, interrupt_in, payload_size))
    
    #         if verbose:
    #             log('Testing OUT/IN data correctness for isochronous endnpoint pair.')
    #         payload_size = 128 # range(1, iso_out.wMaxPacketSize + 1):
    #         try:
    #             self.loopback_ep_test(iso_out, iso_in, payload_size)
    #         except usb.USBError as err:
    #             log(err)
    ...
    def ep_test_halt(self, dev: usb.core.Device, log, verbose=True):
        ...
        for intf in cfg:
            ...
            while time.time() < end_ts:
                self.halt_ep_test(dev, bulk_out, bulk_in, log)
                self.request_endpoint_loops_restart(dev)
    
            if verbose:
                log('Testing endpoint halt at a random point of interrupt transmission.')
            end_ts = time.time() + 1.0
            while time.time() < end_ts:
                self.halt_ep_test(dev, interrupt_out, interrupt_in, log)
                self.request_endpoint_loops_restart(dev)
    
    
    def ep_test_parallel_transfers(self, dev: usb.core.Device, log, verbose=False):
        ...
        for intf in cfg:
            ...
            for kwargs in (test_kwargs_bulk_ep, test_kwargs_interrupt_ep):
                ep_test_threads.append(Thread(target=self.random_size_loopback_ep_test, kwargs=kwargs))
            for t in ep_test_threads:
                t.start()
            for t in ep_test_threads:
                t.join()
            if test_failure.is_set():
                self.raise_unconditionally(self.lineno(), 'Payload mismatch')
            if test_error.is_set():
                self.raise_unconditionally(self.lineno(), 'USBError')
    
    
    def ep_test_parallel_transfers_ctrl(self, dev: usb.core.Device, log, verbose=False):
        ...
        for intf in cfg:
            ...
            while any(t.is_alive() for t in ep_test_threads):
                self.control_stress_test(dev, log)
                self.control_sizes_test(dev, log)
            for t in ep_test_threads:
                t.join()
            if test_failure.is_set():
                self.raise_unconditionally(self.lineno(), 'Payload mismatch')
    ...
    def ep_test_abort(self, dev: usb.core.Device, log, verbose=False):
        ...
        for intf in cfg:
            ...
            for ep_in in (bulk_in, interrupt_in):
                ...
                if FORBIDDEN_PAYLOAD_VALUE in payload_in:
                    self.raise_unconditionally(
                        self.lineno(), 'Endpoint buffer not released when aborting the '
                ...
                if too_little or too_much:
                    self.raise_unconditionally(
                        self.lineno(), 'Invalid size of data successfully received from endpoint '
            ...
            for ep_out in (bulk_out, interrupt_out):
                ...
                while num_bytes_written < payload_size:
                    ...
                    try:
                        ...
                        break
                try:
                    ep_buff_correct = self.request_abort_buff_check(dev, ep_out)
                except (usb.core.USBError, IndexError, TypeError) as err:
                    self.raise_unconditionally(
                        self.lineno(), 'Unable to verify endpoint buffer content ({!r}).'.format(err))
                ...
                if too_little or too_much:
                    self.raise_unconditionally(
                        self.lineno(), 'Invalid size of data successfully sent to endpoint '
                        '{0.bEndpointAddress:#04x} before aborting the transfer. '
                        'Value {1} B out of range [{2}, {3}).'
                        .format(ep_out, num_bytes_written,
    ...
    def ep_test_data_toggle(self, dev: usb.core.Device, log, verbose=False):
        ...
        for intf in cfg:
            ...
            self.loopback_ep_test(bulk_out, bulk_in, bulk_out.wMaxPacketSize)
            ...
            try:
                ...
                self.raise_unconditionally(self.lineno(), 'Data toggle not reset when setting interface.')
            ...
            try:
                ...
                if verbose:
                    log(USB_ERROR_FMT.format(err, bulk_out, bulk_in, bulk_out.wMaxPacketSize))
                self.raise_unconditionally(self.lineno(), 'Data toggle not reset when calling ClearFeature(ENDPOINT_HALT) '
            ...
            try:
                ...
                if verbose:
                    log(USB_ERROR_FMT.format(err, bulk_out, bulk_in, bulk_out.wMaxPacketSize))
                self.raise_unconditionally(self.lineno(), 'Data toggle not reset when clearing endpoint halt.')
    
    
    def device_reset_test(self, log):
        """
    ...
    def device_soft_reconnection_test(self, log):
        """
    ...
    def device_suspend_resume_test(self, log):
        """
    ...
    def repeated_construction_destruction_test(self, log):
        """
    ...
    def release_interfaces(self, dev):
    ...
    def get_status(self, dev: usb.core.Device, recipient, index = 0):
    ...
    def get_descriptor(self, dev: usb.core.Device, type_index, lang_id, length):
