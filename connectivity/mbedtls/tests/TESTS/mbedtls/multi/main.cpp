...
/* Tests several calls to mbedtls_sha256_update that are not modulo 64 bytes. */
void test_case_sha256_split()
{
    ...
    (void)mbedtls_sha256_update(&ctx, test_buf, 168);
#else
    printf("test splitted into 3 pieces\n");
    (void)mbedtls_sha256_update(&ctx, test_buf, 2);
    (void)mbedtls_sha256_update(&ctx, test_buf + 2, 66);
    (void)mbedtls_sha256_update(&ctx, test_buf + 68, 100);
#endif

    (void)mbedtls_sha256_finish(&ctx, outsum);
...
void test_case_sha256_multi()
{
    ...
    //Start both contexts
    (void)mbedtls_sha256_starts(&ctx1, 0);
    (void)mbedtls_sha256_starts(&ctx2, 0);

    printf("upd ctx1\n");
    (void)mbedtls_sha256_update(&ctx1, test_buf, 56);
    printf("upd ctx2\n");
    (void)mbedtls_sha256_update(&ctx2, test_buf, 66);
    printf("finish ctx1\n");
    (void)mbedtls_sha256_finish(&ctx1, outsum1);
    printf("upd ctx2\n");
    (void)mbedtls_sha256_update(&ctx2, test_buf + 66, 46);
    printf("clone ctx2 in ctx3\n");
    mbedtls_sha256_clone(&ctx3, (const mbedtls_sha256_context *)&ctx2);
    printf("free ctx1\n");
    mbedtls_sha256_free(&ctx1);
    printf("upd ctx2\n");
    (void)mbedtls_sha256_update(&ctx2, test_buf + 112, 56);
    printf("upd ctx3 with different values than ctx2\n");
    (void)mbedtls_sha256_update(&ctx3, test_buf2, 56);
    printf("finish ctx2\n");
    (void)mbedtls_sha256_finish(&ctx2, outsum2);
    printf("finish ctx3\n");
    (void)mbedtls_sha256_finish(&ctx3, outsum3);
    printf("free ctx2\n");
